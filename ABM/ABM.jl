...
while t < T+1
    # rates: from eq. 3, first expression. (second expression is in error)
    ...
    # change state
    rand() < π⁻/(π⁻ + π⁺) ? n -=1 : n += 1 # uses prob. that one of two exponentials occurs first
end
