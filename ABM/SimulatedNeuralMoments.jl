...
function MakeNeuralMoments(model::SNMmodel, transf; TrainTestSize=1, Epochs=2000)
    data = 0.0
    datadesign = 0.0
    ...
    # transform stats to robustify against outliers and to lie in [0,1]
    qₗ = [quantile(statistics[:,i], 0.01) for i = 1:size(statistics, 2)]
    qᵤ = [quantile(statistics[:,i], 0.99) for i = 1:size(statistics, 2)]
    nninfo = (qₗ, qᵤ, minimum(statistics, dims=1)[:], maximum(statistics,dims=1)[:])
    for i = 1: size(statistics,1)
        statistics[i,:] .= TransformStats!(statistics[i,:], nninfo)
    end    
    display(dstats(statistics))
    sleep(20)
    # train net
    ...
    nStats = size(xin,1)
    NNmodel = Chain(
        Dense(nStats, 10*nStats, hardtanh),
        Dense(10*nStats, 10*nParams, hardtanh),
